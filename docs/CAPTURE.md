...
- **Quick Properties Bar**: Appears below the toolbar when a drawable tool is active. Shows context-aware controls: shared primary color, text background, blur type, arrow style, watermark text/style/opacity/rotation, stroke width, font size, corner radius. Primary color is remembered across Annotate sessions and reused by drawable tools; Filled Rectangle applies it to both border and fill. Favorite colors are capped at 4 per role to match the visible quick swatches. Numeric defaults such as stroke width, font size, corner radius, and watermark opacity/rotation are shared across compatible tools when no specific annotation is selected; selected-item edits stay local to that annotation.
